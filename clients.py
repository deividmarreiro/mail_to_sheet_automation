...
class GoogleSheetClient:
    ...
    def get_by_sheets_tab(self, sheets_tab: str = None) -> pd.DataFrame:
        ...
        return pd.DataFrame(self.get_spreadsheet().worksheet(sheets_tab).get_all_records())

    def update_all_sheet(self, new_data: pd.DataFrame) -> str:
        spreadsheet = self.get_spreadsheet()
        worksheet = spreadsheet.worksheet(self.sheet_tab)
        existing_records = worksheet.get_all_records()
        existind_data = pd.DataFrame(existing_records) if existing_records else pd.DataFrame()

        updated_data = pd.concat([existind_data, new_data], ignore_index=True)

        results = worksheet.update([updated_data.columns.values.tolist()] + updated_data.values.tolist())
